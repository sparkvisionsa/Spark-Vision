/**
 * يجب أن يبقى متوافقاً مع SparkVision-Backend/src/organization/serial-numbering.ts
 */
...
export function projectSerialLabel(project: {
  ...
  const reference = typeof project.referenceNumber === "string" ? project.referenceNumber.trim() : "";
  if (reference) return reference;
  if (typeof project.displayNumber === "number" && Number.isFinite(project.displayNumber)) {
    return String(project.displayNumber);
  }
  return null;
}
