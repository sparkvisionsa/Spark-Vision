...
/**
 * رسم صفحات PDF المرفوعة — هدف ~300 DPI (72×4.2) لقراءة الأرقام/الجداول الصغيرة
 * بدون بكسلة عند العرض أو التضمين في التقرير.
 */
const PDF_UPLOAD_RENDER_SCALE = 4.2;
/** سقف ذاكرة معقول لصفحة واحدة (~A4 عند 300DPI ≈ 8–12M بكسل؛ نسمح بصفحات أعرض) */
const PDF_UPLOAD_MAX_PAGE_PIXELS = 48_000_000;
/** عند الدقة العالية نقلّل التوازي لتفادي نفاد ذاكرة المتصفح */
const PDF_PARALLEL_PAGES = 3;
/** JPEG عالي جداً — الجداول والأرقام تتضرر بسرعة تحت 0.95 */
const PDF_PAGE_EXPORT_JPEG_QUALITY = 0.97;
...
const EXCEL_AUTO_IMAGE_RENDER_SCALE = 2.35;
const EXCEL_AUTO_IMAGE_JPEG_QUALITY = 0.93;
...
const EXCEL_PDF_COMMIT_RENDER_SCALE = 2.4;
const EXCEL_PDF_COMMIT_JPEG_QUALITY = 0.93;
/**
...
function trimCanvasWhiteMargins(
  ...
  // نسخ 1:1 بدون تنعيم حتى لا تُبهَت حواف الأرقام والنصوص
...
async function renderPdfPageToPngFile(
  ...
  await page.render({
    ...
    canvasContext: ctx,
    viewport,
    // وضع الطباعة يحافظ على وضوح الخطوط والمتجهات أكثر من وضع الشاشة
    intent: "print",
    ...
    for (let start = 1; start <= pageCount; start += PDF_PARALLEL_PAGES) {
      ...
      for (let i = 0; i < rendered.length; i += 1) {
        ...
        const image: MvValuationAccountingImage = {
          ...
          displayWidthPercent: 98,
          displayMaxHeightPx: 2400,
...
function downloadAccountingImage(projectId: string, image: MvValuationAccountingImage, fileName: string) {
  const href = resolveValuationAccountingImageSrc(projectId, image);
  if (!href) return;
...
export default function MvValuationAccountingWorkspace({
  ...
  const ensureImageDataUrl = useCallback(async (image: MvValuationAccountingImage) => {
    ...
    throw new Error("الصورة غير متاحة.");
