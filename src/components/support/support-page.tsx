...
function AgentManagement({ open, onOpenChange, onChanged }: { open: boolean; onOpenChange: (value: boolean) => void; onChanged: () => void }) {
  ...
  const change = async (identifier: { phone: string } | { userId: string }, enabled: boolean) => {
    setBusy(true); setError("");
    try { const data = await api<{ agents: SupportAgent[] }>("/agents", { method: "PATCH", body: JSON.stringify({ ...identifier, enabled }) }); setAgents(data.agents); setPhone(""); onChanged(); }
    catch (cause) { setError(supportError(cause)); } finally { setBusy(false); }
  };
...
export default function SupportPage({ mode = "support", embedded = false, productOverride, compose = false }: { mode?: "support" | "developer"; embedded?: boolean; productOverride?: SupportProduct; compose?: boolean }) {
  ...
  const [status, setStatus] = useState("all"); const kind = developer ? "developer" : "ticket"; const [productFilter, setProductFilter] = useState<string>(product === "general" ? "all" : product);
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      const data = await api<SupportTicketList>(`/tickets?${params}`);
      if (run !== version.current) return;
      setTickets(data.tickets); setCounts(data.counts ?? {}); setTotal(data.total); setHasMore(data.hasMore); setError("");
