...
type Center = { mode: "support" | "developer"; product: SupportProduct };
type SupportContextValue = { summary: SupportSummary; refresh: () => void; openRecorder: () => void; openAssistant: () => void; openSupport: (product?: SupportProduct) => void; openDeveloperRequests: (product?: SupportProduct) => void };
...
export function SupportProvider({ children }: { children: ReactNode }) {
  ...
  const openCenter = useCallback((mode: Center["mode"], product: SupportProduct = "general") => {
    if (!user) { window.dispatchEvent(new CustomEvent("sv:open-auth-modal")); return; }
    setAssistantOpen(false); setRecorderOpen(false); setCenter({ mode, product });
  }, [user?.id]);
  const openSupport = useCallback((product: SupportProduct = "general") => openCenter("support", product), [openCenter]);
  ...
  return <SupportContext.Provider value={value}>
    ...
    {user && center && <SupportCenterModal open onOpenChange={open => { if (!open) setCenter(null); }} mode={center.mode} product={center.product} />}
