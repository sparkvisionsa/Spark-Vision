...
async function proxyPatch(request: NextRequest) {
  ...
  const target = `${mvBackendOriginForProxy()}/api/company/branding${url.search}`;
  ...
  try {
    ...
    res.headers.set("x-spark-proxy", "company-branding");
    for (const c of collectSetCookieHeaders(upstream)) {
      res.headers.append("set-cookie", c);
    }
    return res;
  } catch (err) {
    console.error("[api/company/branding proxy] upstream failed", err);
